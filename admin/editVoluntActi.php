...
{	
	if(!isset($_POST['mName']) || strlen($_POST['mName'])<1)
	{
        ...
        if(!isset($_POST['from']) || strlen($_POST['from'])<1)
	{
		//required variables are empty
		die("Date is empty!");
	}
	...
	if(!isset($_FILES['mFile']))
	{
            ...
            $query = mysql_query("UPDATE volunteer_activity SET title='{$FileTitle}' , from_date='{$_POST['from']}' ,to_date ='{$_POST['to']}', discr = '{$_POST['descr']}',Number_of_people='{$_POST['number']}'  WHERE id='{$_POST['id']}'")or die(mysql_error());
            die("success");
	}else{
...
//   Rename and save uploded file to destination folder.
   if(move_uploaded_file($_FILES['mFile']["tmp_name"], $UploadDirectory . $NewFileName ))
   {
	require_once('../ImageWorkshop/src/PHPImageWorkshop/ImageWorkshop.php');
        ...
        $query = mysql_query("UPDATE volunteer_activity SET title='{$FileTitle}' ,image='{$filename}', from_date='{$_POST['from']}' ,to_date ='{$_POST['to']}', discr = '{$_POST['descr']}',Number_of_people='{$_POST['number']}' WHERE id='{$_POST['id']}'")or die(mysql_error());
         die("success");
   }else{
