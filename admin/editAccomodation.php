...
{	
	if(!isset($_POST['mName']) || strlen($_POST['mName'])<1)
	{
        ...
        if(!isset($_POST['rooms']) || strlen($_POST['rooms'])<1)
	{
		//required variables are empty
		die("Number of room is empty!");
	}
        if(!isset($_POST['price']) || strlen($_POST['price'])<1)
	{
		//required variables are empty
		die("Price Range is empty!");
	}
        
	if(!isset($_FILES['mFile']))
	{
            $FileTitle= mysql_real_escape_string($_POST['mName']); // file title
            $query = mysql_query("UPDATE rooms SET name='$FileTitle',price='{$_POST['price']}',rooms = '{$_POST['rooms']}',discr = '{$_POST['descr']}',status ='{$_POST['status']}' WHERE id='{$_POST['id']}'")or die(mysql_error());
            die("success");
	}else{
...
//   Rename and save uploded file to destination folder.
   if(move_uploaded_file($_FILES['mFile']["tmp_name"], $UploadDirectory . $NewFileName ))
   {
	require_once('../ImageWorkshop/src/PHPImageWorkshop/ImageWorkshop.php');
       ...
       $query = mysql_query("UPDATE rooms SET name='$FileTitle',price='{$_POST['price']}',rooms = '{$_POST['rooms']}',discr = '{$_POST['descr']}',status ='{$_POST['status']}',image='{$filename}' WHERE id='{$_POST['id']}'")or die(mysql_error());
       die("success");
   }else{
